...
if (fs::dir_exists(cached_dataset)) {
  ...
  if (has_main) {
    has_main <- length(fs::dir_ls("./results", glob = "*.parquet", recurse = TRUE)) > 0
  }
  ...
  if (has_main) {
    ...
    full_data <-
      ...
      dplyr::mutate(
        # Harmonize parameter names: translate ACML names to Stan names
        parameter = bayes2stage::translate_parameter_names(
          parameter,
          from = "acml",
          to = "stan"
        ),
    ...
    cli::cli_alert_success("Loaded {.val {nrow(full_data)}} rows from {.path ./results}")
  } else {
...
if (nrow(all_params) < original_count) {
  ...
  missing <- dplyr::anti_join(
    get_all_true_parameter_values(),
    available_combos,
    by = c("sim_setting", "parameter")
  )
  missing_params <- unique(missing$parameter)

  cli::cli_alert_info(
    "Skipping {.val {original_count - nrow(all_params)}} parameter combinations not in data: {.val {missing_params}}"
  )
}

if (nrow(all_params) == 0) {
  cli::cli_abort("No matching (sim_setting, parameter) combinations found in data!")
}
...
if (length(plot_files) > 0) {
  ...
  magick::image_write(images, "plots/all_parameters_combined.pdf", format = "pdf")
