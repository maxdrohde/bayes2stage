...
get_parameter_registry <- function() {
    ...
    # Note: phi has two entries to handle both naming conventions
    result <- tibble::tribble(
        ...
        "beta_t_x_interaction", "beta_x_t_interaction", "Beta x:t Interaction",   "beta_t:x",
...
#' MCMC diagnostics and model summaries. When `available` is provided,
#' also includes all indexed `beta[i]` and `gamma[i]` parameters found
#' in the available set (useful when models have multiple covariates).
#'
#' @param available Optional character vector of available parameter names
#'   from a fitted model. When provided, all `beta[i]` and `gamma[i]`
#'   parameters are included.
#' @return Character vector of Stan parameter names
#' @export
get_key_parameters <- function(available = NULL) {
    registry <- get_parameter_registry()
    result <- registry$stan_name[!is.na(registry$stan_name)]

    if (!is.null(available)) {
        indexed <- grep("^(beta|gamma)\\[\\d+\\]$", available, value = TRUE)
        result <- unique(c(result, indexed))
    }
