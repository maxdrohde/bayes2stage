...
LogLikeC2 <- function(y, x, z, w.function, id, beta, sigma.vc, rho.vc, sigma.e, cutpoints, SampProb, Weights, Keep.liC = FALSE) {
    ...
    if (Keep.liC == FALSE) {
        out <- -1 * Reduce("+", liC.and.logACi)[1] ## sum ss contributions to liC
    } else {
        out <- list(
            liC = c(unlist(sapply(liC.and.logACi, function(x) x[1]))), ## ss contributions to liC
            logACi = c(unlist(sapply(liC.and.logACi, function(x) x[2])))
        )
    } ## ss contributions to ACi
    out
}
...
LogLikeiC2 <- function(subjectData, beta, sigma.vc, rho.vc, sigma.e) {
    ...
    if (!(w.function %in% c("bivar", "mvints", "mvslps"))) {
        if (w.function %in% c("intercept", "intercept1")) {
            wi <- wi.tmp[1, ]
        } else if (w.function %in% c("slope", "slope1")) {
            wi <- wi.tmp[2, ]
        } else if (w.function %in% c("intercept2")) {
            wi <- wi.tmp[3, ]
        } else if (w.function %in% c("slope2")) {
            wi <- wi.tmp[4, ]
        } else if (w.function == "mean") {
            wi <- t(rep(1 / ni, ni))
        }
        wi <- matrix(wi, 1, ni)
        ...
        if (w.function %in% c("bivar")) {
            wi <- wi.tmp[c(1, 2), ]
        } else if (w.function %in% c("mvints")) {
            wi <- wi.tmp[c(1, 3), ]
        } else if (w.function %in% c("mvslps")) {
            wi <- wi.tmp[c(2, 4), ]
        }
...
logACi1q.score2 <- function(subjectData, beta, sigma.vc, rho.vc, sigma.e) {
    ...
    if (w.function %in% c("intercept", "intercept1")) {
        wi <- (solve(t.zi %*% zi) %*% t.zi)[1, ]
    } else if (w.function %in% c("slope", "slope1")) {
        wi <- (solve(t.zi %*% zi) %*% t.zi)[2, ]
    } else if (w.function %in% c("intercept2")) {
        wi <- (solve(t.zi %*% zi) %*% t.zi)[3, ]
    } else if (w.function %in% c("slope2")) {
        wi <- (solve(t.zi %*% zi) %*% t.zi)[4, ]
    } else if (w.function == "mean") {
        wi <- t(rep(1 / ni, ni))
    }
    wi <- matrix(wi, 1, ni)
...
li.lme.score2 <- function(subjectData, beta, sigma.vc, rho.vc, sigma.e) {
    ...
    list(
        gr = append(dli.dbeta, c(dli.dsigma.vc, dli.drho.vc, dli.dsigma.e)) * Weights.i,
        vi = vi
    )
}
...
LogLikeC.Score2 <- function(y, x, z, w.function, id, beta, sigma.vc, rho.vc, sigma.e, cutpoints, SampProb, Weights, CheeseCalc = FALSE) {
    ...
    if (CheeseCalc == TRUE) {
        if (!(w.function[[1]] %in% c("bivar", "mvints", "mvslps"))) {
            ...
            GradiMat <- mapply("-", Gradienti, logACi.Score)
        } ## notice this has the opposite sign compared to above.  Remember to check
...
LogLikeCAndScore2 <- function(params, y, x, z, id, w.function, cutpoints, SampProb, Weights, ProfileCol = NA, Keep.liC = FALSE) {
    ...
    out <- LogLikeC2(
        y = y, x = x, z = z, w.function = w.function, id = id, beta = beta, sigma.vc = sigma.vc, rho.vc = rho.vc, sigma.e = sigma.e, cutpoints = cutpoints,
        SampProb = SampProb, Weights = Weights, Keep.liC = Keep.liC
    )
    GRAD <- LogLikeC.Score2(
        y = y, x = x, z = z, w.function = w.function, id = id, beta = beta, sigma.vc = sigma.vc, rho.vc = rho.vc, sigma.e = sigma.e, cutpoints = cutpoints,
...
acml.lmem2 <- function(formula.fixed,
    ...
    if (is.null(formula.random)) {
        stop("Specify the random effects portion of the model.  It is currently NULL.")
    }
    ...
    terms <- unique(c(
        all.vars(formula.fixed), all.vars(formula.random),
    ...
    acml.fit <- nlm(LogLikeCAndScore2, InitVals,
        y = y, x = x, z = z, id = id, w.function = w.function,
    ...
    Cheese <- LogLikeC.Score2(
        y = y, x = x, z = z, w.function = w.function,
